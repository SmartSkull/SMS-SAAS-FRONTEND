...
export function Navbar({ scrolled, menu, setMenu }: NavbarProps) {
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
      scrolled
        ? 'bg-white/95 backdrop-blur-md shadow-[0_2px_20px_rgba(0,0,0,.06)] border-b border-gray-100'
        : 'bg-transparent'
    } md:bg-transparent md:border-0`}>
      <div className="max-w-7xl mx-auto px-6 h-[68px] flex items-center justify-between bg-black md:bg-transparent">
      ...
      <div className="md:hidden overflow-hidden transition-all duration-250 ease-in-out" style={{ maxHeight: menu ? '300px' : '0px', opacity: menu ? 1 : 0 }}>
        <div className="px-6 pb-6 pt-2 border-t border-gray-800 bg-black flex flex-col gap-5">
          {NAV.map(([l, h]) => (
            <a key={l} href={h} onClick={() => setMenu(false)} className="font-semibold text-white">{l}</a>
          ))}
          <div className="flex gap-3">
            <Link href="/school" onClick={() => setMenu(false)} className="flex-1 text-center border border-gray-700 py-3 rounded-xl font-semibold text-sm text-white">Log in</Link>
