...
import { X, Camera, ScanFace, CheckCircle, AlertCircle, Loader2, UserPlus, RefreshCw } from 'lucide-react';
import { api, endpoints } from '@/lib/api';
import clsx from 'clsx';

type Stage =
  | 'ready'        // camera live, waiting for capture
  | 'capturing'    // taking photo
  | 'recognizing'  // calling backend
  | 'not_enrolled' // face not found — prompt to enroll
  | 'enrolling'    // sending enroll request
  | 'success'      // clocked in
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      if (res?.enrolled === false) {
        ...
        setMessage(res.message ?? 'Face not registered.');
        return;
      }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100"
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-white/70">Look at the camera to mark attendance</p>
            </div>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="relative rounded-2xl overflow-hidden bg-black aspect-[4/3]">
            ...
            <video
              ref={videoRef}
              className={clsx('w-full h-full object-cover', !showCamera && 'hidden')}
          ...
          <div className="space-y-2.5">
            {/* Clock In button — shown when camera is live */}
            {stage === 'ready' && (
            ...
            {/* Enroll button — shown when face not found */}
            {stage === 'not_enrolled' && (
              <>
            ...
            {/* Retry button — shown on error or after enrollment */}
            {(stage === 'error' || stage === 'enrolled') && (
              <button onClick={stage === 'enrolled' ? handleClockIn : retry}
                ...
                style={{ background: primaryColor }}>
                {stage === 'enrolled' ? <><Camera size={18} /> Clock In Now</> : <><RefreshCw size={16} /> Try Again</>}
              </button>
            )}
