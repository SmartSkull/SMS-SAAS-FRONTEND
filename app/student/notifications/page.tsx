...
export default function NotificationsPage() {
  ...
  const load = () => {
    setLoading(true);
    api.get<{ data: Notification[] }>(endpoints.student.notifications)
  ...
  const markAllRead = async () => {
    try {
      await api.post(endpoints.student.notificationsRead, {});
      ...
      <div className="bg-white rounded-2xl card shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-gray-50">
            {items.map(n => (
              <div key={n.id} className={`flex gap-4 p-4 ${!n.readAt ? 'bg-blue-50/50' : ''}`}>
                <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${!n.readAt ? 'bg-blue-100' : 'bg-gray-100'}`}>
                  <Bell size={16} className={!n.readAt ? 'text-blue-600' : 'text-gray-400'} />
                </div>
