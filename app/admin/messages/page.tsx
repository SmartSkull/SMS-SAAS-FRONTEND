...
export default function AdminMessages() {
  ...
  const loadConvos = useCallback(async (quiet = false) => {
    ...
    try {
      const r = await api.get<ApiResponse<any[]>>(MESSAGES_EP);
      setConvos(r.data ?? []);
    } catch {
  ...
  const sendFile = async (file: File, caption = '') => {
    ...
    try {
      ...
      const res = await api.upload<ApiResponse<{ url: string }>>(endpoints.admin.messagesUpload, form);
      const fileUrl = res.data?.url ?? '';
      setMessages(p => p.map(m => m.id === tmpId ? { ...m, file_url: fileUrl || localUrl } : m));
      await api.post(MESSAGES_EP, { receiver_id: active, message: caption || file.name, file_url: fileUrl });
      openConvo(active);
      loadConvos(true);
    } catch {
            ...
            <div className="bg-white border-t border-gray-100">
              ...
              <form onSubmit={sendMessage} className="px-4 py-3 flex gap-2">
                <button type="button" onClick={() => setShowAttach(v => !v)} className="p-2.5 text-gray-400 hover:text-gray-600 rounded-xl">
                  {uploading ? <Loader2 size={18} className="animate-spin text-gray-400" /> : <Paperclip size={18} color={showAttach ? '#9333ea' : '#64748b'} />}
                </button>
                ...
                <VoiceRecorder onSend={(file) => sendFile(file)} accentColor="bg-purple-600" />
